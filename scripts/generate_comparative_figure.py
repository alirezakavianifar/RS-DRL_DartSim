...
def generate_comparative_figure():
    ...
    methods = ['Moreno 2019\n(Reactive)', 'Kinneer 2021\n(SASS/PLA)', 'Baseline DQN\n(rho=0)', 'RS-DRL\n(Proposed)']
    success_rates = [62.0, 85.0, 46.7, 89.5]
    survival_rates = [62.0, 85.0, 70.0, 92.5]
    latencies_ms = [5.4, 4500.0, 0.037, 0.037] # Decision time
    utility_scores = [0.600, 0.780, 0.450, 0.895]
    ...
    for bar in bars1:
        yval = bar.get_height()
        axs[0, 0].text(bar.get_x() + bar.get_width()/2.0, yval + 2, f"{yval:.1f}%", ha='center', va='bottom', fontsize=10, fontweight='bold')
    ...
    for bar in bars2:
        yval = bar.get_height()
        axs[0, 1].text(bar.get_x() + bar.get_width()/2.0, yval + 2, f"{yval:.1f}%", ha='center', va='bottom', fontsize=10, fontweight='bold')
    ...
    axs[1, 0].set_ylim(0.001, 20000)
    axs[1, 0].grid(axis='y', linestyle='--', alpha=0.5)
    for bar, val in zip(bars3, latencies_ms):
        axs[1, 0].text(bar.get_x() + bar.get_width()/2.0, val * 1.5, f"{val:.3f} ms" if val < 1 else f"{val:.1f} ms", ha='center', va='bottom', fontsize=10, fontweight='bold')
    ...
    for bar in bars4:
        yval = bar.get_height()
        axs[1, 1].text(bar.get_x() + bar.get_width()/2.0, yval + 0.02, f"{yval:.3f}", ha='center', va='bottom', fontsize=10, fontweight='bold')
